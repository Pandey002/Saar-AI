...
export function SidebarPanel({ title, subtitle, items, onDownloadPdf }: SidebarPanelProps) {
  return (
    <aside className="sidebar-panel space-y-4">
      <div className="rounded-[24px] border border-line bg-[#F6F3E6] p-5 shadow-card">
        <p className="sidebar-panel-eyebrow text-[11px] font-semibold uppercase tracking-[0.12em] text-primary">Live Breakdown</p>
        <h3 className="mt-2 text-[18px] font-semibold tracking-[-0.03em] text-ink">{title}</h3>
        <p className="mt-2 text-sm leading-6 text-muted">{subtitle}</p>
        <div className="mt-4 space-y-3">
          {items.map((item) => (
            <div key={`${item.label}-${item.value}`} className="flex items-center justify-between rounded-2xl bg-black/5 px-4 py-3">
      ...
      <div className="sidebar-panel-actions rounded-[24px] border border-line bg-[#F6F3E6] p-5 shadow-card">

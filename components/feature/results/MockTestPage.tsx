...
export function MockTestPage({
  ...
  function handleDownloadPdf() {
    ...
    const printableWindow = window.open("", "_blank", "noopener,noreferrer,width=1000,height=800");
    if (!printableWindow) {
      return;
    }
    ...
    const reviewRows = questions.map((question) => {
      ...
      return `<article style="border:1px solid #e2e8f0;border-radius:16px;padding:16px;margin-bottom:16px;"><div style="display:flex;justify-content:space-between;gap:16px;"><strong>${escapeHtml(question.question)}</strong><span>${result ? `${result.score}/${result.maxScore}` : "Pending"}</span></div><p style="margin:10px 0 0;color:#475569;"><strong>Your answer:</strong> ${escapeHtml(result?.userAnswer || "Unattempted")}</p><p style="margin:6px 0 0;color:#0f172a;"><strong>Correct answer:</strong> ${escapeHtml(result?.correctAnswer || "")}</p><p style="margin:6px 0 0;color:#475569;">${escapeHtml(result?.feedback || "")}</p></article>`;
    }).join("");

    printableWindow.document.write(`<html><head><title>${escapeHtml(data.title)} Result</title><style>body{font-family:Arial,sans-serif;margin:32px;color:#0f172a}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;margin:24px 0}.card{border:1px solid #cbd5e1;border-radius:18px;padding:16px}.muted{color:#475569}</style></head><body><h1>${escapeHtml(data.title)}</h1><p class="muted">${escapeHtml(evaluation.summary)}</p><div class="grid"><div class="card"><strong>Score</strong><div>${evaluation.totalScore}/${evaluation.totalMarks}</div></div><div class="card"><strong>Accuracy</strong><div>${evaluation.accuracy}%</div></div><div class="card"><strong>Attempted</strong><div>${evaluation.attempted}/${evaluation.totalQuestions}</div></div></div><h2>AI Analysis</h2><p>${escapeHtml(evaluation.analysis.summary)}</p><h3>Strengths</h3><ul>${evaluation.analysis.strengths.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><h3>Weaknesses</h3><ul>${evaluation.analysis.weaknesses.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><h3>Suggestions</h3><ul>${evaluation.analysis.suggestions.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><h2>Question Review</h2>${reviewRows}</body></html>`);
